...
class Bot_ChuckwallaChatClientModel extends ChuckwallaBaseModel implements AgaviISingletonModel 
{
	...
	public function onChannelTopic(IRCConnection $connection, $parameters, IRCInboundMessage $message)
	{
		...
		$channel->setTopic(utf8_encode(substr($msg->params[2], 0, 255)));
		$channel->save();
	}
	...
	{
		if(isset($this->connectionConfig['channels']) && is_array($this->connectionConfig['channels'])) {
			foreach($this->connectionConfig['channels'] as $channel) {
				$this->join($channel);
			}
		}
	}
